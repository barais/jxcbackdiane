...
public class ArticlesResource {
	...
	public ArticlesResource(ArticlesService articlesService, @NoWrapRootValueObjectMapper ObjectMapper objectMapper) {
	...
	public Response feed(@QueryParam("offset") int offset, @QueryParam("limit") int limit,
		...
		return Response.ok(objectMapper.writeValueAsString(new ArticlesResponse(result))).status(Response.Status.OK)
				.build();
	}
	...
	public Response getArticles(@QueryParam("offset") int offset, @QueryParam("limit") int limit,
			@QueryParam("tag") List<String> tags, @QueryParam("author") List<String> authors,
			@QueryParam("favorited") List<String> favorited, @Context SecurityContext securityContext)
			throws JsonProcessingException {
		Long loggedUserId = getLoggedUserId(securityContext);
		ArticlesData result = articlesService.findArticles(offset, limit, loggedUserId, tags, authors, favorited);
		return Response.ok(objectMapper.writeValueAsString(new ArticlesResponse(result))).status(Response.Status.OK)
				.build();
	}
	...
	public Response getAllArticles(@Context SecurityContext securityContext) {
		...
		List<ArticleData> articles = articlesService.getAll(loggedUserId);
		List<ArticleResponse> articlesr = new ArrayList<>();
		for (ArticleData a : articles) {
			ArticleResponse resp = new ArticleResponse(a);			
			articlesr.add(resp);
		}
		return Response.ok(articlesr).status(Response.Status.OK).build();
	}
	...
	public Response create(
			@Valid @NotNull(message = ValidationMessages.REQUEST_BODY_MUST_BE_NOT_NULL) NewArticleRequest newArticleRequest,
			@Context SecurityContext securityContext) {
		Long loggedUserId = getLoggedUserId(securityContext);
		ArticleData newArticleData = articlesService.create(newArticleRequest.getTitle(),
				newArticleRequest.getDescription(), newArticleRequest.getBody(), newArticleRequest.getTagList(),
				loggedUserId);
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response update(@PathParam("slug") @NotBlank String slug,
			@Valid @NotNull UpdateArticleRequest updateArticleRequest, @Context SecurityContext securityContext) {
		Long loggedUserId = getLoggedUserId(securityContext);
		ArticleData updatedArticleData = articlesService.update(slug, updateArticleRequest.getTitle(),
				updateArticleRequest.getDescription(), updateArticleRequest.getBody(), loggedUserId);
	...
	public Response getCommentsBySlug(
		...
		return Response.ok(objectMapper.writeValueAsString(new CommentsResponse(comments))).status(Response.Status.OK)
				.build();
	}
	...
	public Response createComment(
			@PathParam("slug") @NotBlank(message = ValidationMessages.SLUG_MUST_BE_NOT_BLANK) String slug,
			@Valid NewCommentRequest newCommentRequest, @Context SecurityContext securityContext) {
		Long loggedUserId = getLoggedUserId(securityContext);
		CommentData commentData = articlesService.createComment(slug, newCommentRequest.getBody(), loggedUserId);
