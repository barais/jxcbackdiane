...
public class ArticlesServiceImpl implements ArticlesService {
	...
	public ArticlesServiceImpl(ArticlesUsersRepository articlesUsersRepository,
			ArticlesTagsRepository articlesTagsRepository, UserRepository userRepository, TagRepository tagRepository,
			ArticleRepository articleRepository, CommentRepository commentRepository, ProfilesService profilesService,
	...
	public ArticlesData findRecentArticles(Long loggedUserId, int offset, int limit) {
		...
		List<Article> articles = articleRepository.findMostRecentArticles(loggedUserId, offset, getLimit(limit));
	...
	@Override
	@Transactional
	public ArticlesData findArticles(int offset, int limit, Long loggedUserId, List<String> tags, List<String> authors,
			List<String> favorited) {

		List<Article> articles = articleRepository.findArticles(offset, getLimit(limit), tags, authors, favorited);
	...
	@Override
	@Transactional
	public ArticleData create(String title, String description, String body, List<String> tagList, Long authorId) {
	...
	@Override
	@Transactional
	public ArticleData update(String slug, String title, String description, String body, Long authorId) {
	...
	public void delete(String slug, Long authorId) {
		Article article = articleRepository.findByIdAndSlug(authorId, slug).orElseThrow(ArticleNotFoundException::new);
	...
	public List<CommentData> findCommentsBySlug(String slug, Long loggedUserId) {
		...
		return comments.stream().map(comment -> {
			ProfileData author = profilesService.getProfile(comment.getAuthor().getUsername(), loggedUserId);
	...
	public ArticleData favoriteArticle(String slug, Long loggedUserId) {
		...
		if (!articlesUsersRepository.isFavorited(article.getId(), loggedUserId)) {
			...
			User loggedUser = userRepository.findUserById(loggedUserId).orElseThrow(UserNotFoundException::new);
	...
	public ArticleData unfavoriteArticle(String slug, Long loggedUserId) {
		...
		if (articlesUsersRepository.isFavorited(article.getId(), loggedUserId)) {
			...
			ArticlesUsers articlesUsers = articlesUsersRepository.findById(getArticlesUsersKey(article, loggedUser))
	...
	private CommentData getComment(Comment comment, ProfileData authorProfile) {
		return new CommentData(comment.getId(), comment.getCreatedAt(), comment.getUpdatedAt(), comment.getBody(),
				authorProfile);
	}
	...
	private List<ArticleData> toResultList(List<Article> articles, Long loggedUserId) {
		return articles.stream().map(article -> getArticle(article, loggedUserId)).collect(Collectors.toList());
	}
	...
	private ArticleData getArticle(Article article, Long loggedUserId) {
		...
		List<String> tags = tagRepository.findArticleTags(article.getId()).stream().map(Tag::getName)
				.collect(Collectors.toList());
		return new ArticleData(article.getSlug(), article.getTitle(), article.getDescription(), article.getBody(), tags,
				isFavorited, favoritesCount, article.getCreatedAt(), article.getUpdatedAt(), author);
	}
	...
	@Override
	public List<ArticleData> getAll(long loggedUserId) {
